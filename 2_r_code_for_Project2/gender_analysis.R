...
# Add convalesced status and sex combination. ----
filteredData <- filteredData |>
  mutate(isConvalescedLabelDose = str_c(isConvalescedLabel, '', sex))  
...
model1o <- lmer(nabFlowT ~ poly(ysfdc, 1, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model2o <- lmer(nabFlowT ~ poly(ysfdc, 2, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model3o <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model4o <- lmer(nabFlowT ~ poly(ysfdc, 4, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model5o <- lmer(nabFlowT ~ poly(ysfdc, 5, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model6o <- lmer(nabFlowT ~ poly(ysfdc, 6, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)
...
model3o.0 <- lm(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex,
               data = filteredData)
modelVariances[1] <- summary(model3o.0)$sigma^2

model3o.1 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex +
                (1 | subject),
               data = filteredData,
               REML = TRUE)  
modelVariances[2] <- summary(model3o.1)$sigma^2

model3o.2Uc <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex +
                (1 | subject) + (0 + ysfdc | subject),
               data = filteredData,
               REML = TRUE)
modelVariances[3] <- summary(model3o.2Uc)$sigma^2

model3o.2 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = TRUE)
...
model3 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * sex +
                    (1 + ysfdc | subject),
                  data = filteredData, 
                  REML = FALSE)
...
model1 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * sex +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)
...
optimalModel <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + isConvalesced:sex:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + sex:ysfdc3 + isConvalesced:sex:ysfdc3 + 
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = TRUE)

# Mess with this
m1 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + isConvalesced:sex:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + sex:ysfdc3 + isConvalesced:sex:ysfdc3 + 
                    (1 + ysfdc | subject),
...
m2 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + isConvalesced:sex:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + sex:ysfdc3 + 
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m3 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + isConvalesced:sex:ysfdc2 +
...
m4 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + isConvalesced:sex:ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m5 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + sex:ysfdc2 + 
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m6 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
...
m7 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + isConvalesced:sex:ysfdc +
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m8 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + isConvalesced:ysfdc + sex:ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
...
m9 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
...
m10 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + isConvalesced:sex +
                    ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m11 <- lmer(nabFlowT ~ 1 + isConvalesced + sex + 
                    ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
optimalModel <- lmer(nabFlowT ~ 1 + sex + sex + sex:sex +
                    ysfdc + sex:ysfdc + sex:ysfdc + sex:sex:ysfdc +
                    ysfdc2 + sex:ysfdc2 + sex:ysfdc2 + sex:sex:ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = TRUE)
...
ConfData <- ConfData |>
  ...
  mutate(labels = str_c(isConvalescedLabels, '', sex)) 

xTicks = seq(30, 450, by = 30)
xTicksA = seq(30, 450, by = 60)
...
plot3 <- ggplot(ConfData, aes(x = ysfdc, y = EstimatePN)) +
  geom_line(aes(color = sex),
            show.legend = TRUE) +
  geom_ribbon(aes(ymin = lwrPN, ymax = uprPN, color = sex), 
              fill = 'gray', 
              alpha = 0.4, 
              linetype = 0, 
              show.legend = FALSE) +
  ...
  theme(legend.position = "bottom",
        strip.background = element_blank())   
  scale_color_manual(values = c("M" = "#4DAF4A",
                                "F" = "#984EA3"))    
