...
# Add convalesced status and dose1Type combination. ----
filteredData <- filteredData |>
  mutate(isConvalescedLabelDose = str_c(isConvalescedLabel, '', dose1Type))  
...
ggplot(filteredData, aes(x = dsfd, y = nabFlow, 
  ...
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 30/365) +
  geom_hline(yintercept = 0) +
  facet_wrap(~ dose1Type) +
...
model1o <- lmer(nabFlowT ~ poly(ysfdc, 1, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model2o <- lmer(nabFlowT ~ poly(ysfdc, 2, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model3o <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model4o <- lmer(nabFlowT ~ poly(ysfdc, 4, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model5o <- lmer(nabFlowT ~ poly(ysfdc, 5, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)

model6o <- lmer(nabFlowT ~ poly(ysfdc, 6, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = FALSE)
...
diagnosticPlots <- check_model(model3o, check = c("linearity", "homogeneity","reqq", "qq"))
diagnosticPlots
...
model3o.0 <- lm(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type,
               data = filteredData)
modelVariances[1] <- summary(model3o.0)$sigma^2

model3o.1 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type +
                (1 | subject),
               data = filteredData,
               REML = TRUE)  
modelVariances[2] <- summary(model3o.1)$sigma^2

model3o.2Uc <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type +
                (1 | subject) + (0 + ysfdc | subject),
               data = filteredData,
               REML = TRUE)
modelVariances[3] <- summary(model3o.2Uc)$sigma^2

model3o.2 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type +
                (1 + ysfdc | subject),
               data = filteredData,
               REML = TRUE)
...
model3 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced * dose1Type +
                    (1 + ysfdc | subject),
                  data = filteredData, 
                  REML = FALSE)

model2 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * isConvalesced +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

model1 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE) * dose1Type +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

model0 <- lmer(nabFlowT ~ poly(ysfdc, 3, raw = TRUE)  +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)
...
optimalModel <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + dose1Type:ysfdc3 + isConvalesced:dose1Type:ysfdc3 + 
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = TRUE)

# Mess with this
m1 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + dose1Type:ysfdc3 + isConvalesced:dose1Type:ysfdc3 + 
                    (1 + ysfdc | subject),
...
m2 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + dose1Type:ysfdc3 + 
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m3 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 + isConvalesced:ysfdc3 + 
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m4 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m5 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + 
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m6 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
                  REML = FALSE)

m7 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m8 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
...
m9 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
                  data = filteredData,
...
m10 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m11 <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + 
                    ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
m12 <- lmer(nabFlowT ~ 1 + isConvalesced + 
                    ysfdc + 
                    ysfdc2 +
                    ysfdc3 +
                    (1 + ysfdc | subject),
...
# Originally here, so don't mess with this. 
optimalModel <- lmer(nabFlowT ~ 1 + isConvalesced + dose1Type + isConvalesced:dose1Type +
                    ysfdc + isConvalesced:ysfdc + dose1Type:ysfdc + isConvalesced:dose1Type:ysfdc +
                    ysfdc2 + isConvalesced:ysfdc2 + dose1Type:ysfdc2 + isConvalesced:dose1Type:ysfdc2 +
                    ysfdc3 +
...
ConfData <- data.frame(ysfdc = rep(xPred, 4), 
                       ysfdc2 = rep(xPred^2, 4),
                       ysfdc3 = rep(xPred^3, 4),
                       isConvalesced = rep(c("Yes", "No", "Yes", "No"), each = 100),
                       dose1Type = rep(c("Moderna", "Pfizer"), each = 200))
ConfData <- ConfData |>
...
ConfData <- ConfData |>
  ...
  mutate(isConvalescedLabels = rep(c("Convalesced", "Naive", "Convalesced", "Naive"), each = 100)) |>
  mutate(labels = str_c(isConvalescedLabels, '', dose1Type)) 

xTicks = seq(30, 450, by = 30)
xTicksA = seq(30, 450, by = 120)
ysfdcTicks = (xTicks/365) - mean(filteredData$ysfd)
ysfdcTicksA = (xTicksA/365) - mean(filteredData$ysfd)

filteredData <- filteredData |>
  mutate(isConvalescedLabels = ifelse(isConvalesced == "Yes", "Convalesced", "Naive"))

plot1 <- ggplot(ConfData, aes(x = ysfdc, y = EstimatePN, color = labels)) +
  geom_line(show.legend = FALSE) +
  geom_ribbon(aes(ymin = lwrPN, ymax = uprPN, fill = labels), 
              alpha = 0.4, linetype = 0, show.legend = FALSE) +
  geom_point(data = filteredData, aes(x = ysfdc, y = nabFlow, color = isConvalescedLabelDose), show.legend = FALSE) +
  scale_x_continuous(breaks = ysfdcTicks, labels = xTicks) +
  labs(x = "days since first dose", y = "NAb activity (% neutralization)", color = NULL) +
  theme_classic() 

plot2 <- ggplot(ConfData, aes(x = ysfdc, y = EstimatePN)) +
  geom_line(aes(color = isConvalescedLabels),
            show.legend = TRUE) +
  geom_ribbon(aes(ymin = lwrPN, ymax = uprPN, color = isConvalescedLabels), 
              fill = 'gray', 
              alpha = 0.4, 
              linetype = 0, 
              show.legend = FALSE) +
  geom_point(data = filteredData, 
             aes(x = ysfdc, 
                 y = nabFlow, 
                 color = isConvalescedLabels)) +
  facet_wrap(~ dose1Type) +      
  scale_x_continuous(breaks = ysfdcTicksA, labels = xTicksA) +  
  labs(x = "days since first dose", y = "NAb activity (% neutralization)", color = NULL) +
  theme_classic() +
  theme(legend.position = "bottom",
        strip.background = element_blank()) +
  scale_color_manual(values = c("Convalesced" = "#E41A1C", 
                                "Naive" = "#377EB8")) +
  theme(text = element_text(size = 20))

plot3 <- ggplot(ConfData, aes(x = ysfdc, y = EstimatePN)) +
  geom_line(aes(color = dose1Type),
            show.legend = TRUE) +
  geom_ribbon(aes(ymin = lwrPN, ymax = uprPN, color = dose1Type), 
              fill = 'gray', 
              alpha = 0.4, 
              linetype = 0, 
              show.legend = FALSE) +
  geom_point(data = filteredData, 
             aes(x = ysfdc, 
                 y = nabFlow,
                 color = dose1Type)) +
  facet_wrap(~ isConvalescedLabels) +      
  scale_x_continuous(breaks = ysfdcTicksA, labels = xTicksA) +  
  labs(x = "days since first dose", y = "NAb activity (% neutralization)", color = NULL) +
  theme_classic() +
  theme(legend.position = "bottom",
        strip.background = element_blank()) +  
  scale_color_manual(values = c("Moderna" = "#4DAF4A",
                                "Pfizer" = "#984EA3")) +
  theme(text = element_text(size = 20))

plot_grid(plot2, plot3,
          labels = c("(i)", "(ii)"),
          hjust = 0.03)